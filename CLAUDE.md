...
**Retrieval tracing (LangSmith).** When `LANGSMITH_API_KEY` is set, each turn's retrieval is also sent to LangSmith as a `retriever` run with the query embedding and the pgvector search as child spans, tagged with the call and turn IDs above. It exists for the questions a log line cannot answer — sorting turns by worst score, comparing phrasings, and seeing which of the two legs a slow turn spent itself in (the hosted embedding provider is routinely 95%+ of it).

The caller's question and the chunk text are **withheld by default** and must stay that way anywhere real calls land: section 27's rule against logging transcript text and document contents applies with more force to a third-party service, not less. What goes out is scores, identifiers, lengths, counts and timings — the same shape the log line takes. `LANGSMITH_TRACE_TEXT=true` opts in, for debugging a corpus you own with callers who are you.

Nothing in `app/services/retrieval_tracing.py` may raise or block: retrieval is inside the turn's latency budget, and an observability backend having a bad day must not cost a caller their answer. Note also that LangSmith validates `run_type` server-side and rejects the **entire ingest batch** containing a bad value — one mistyped span silently takes the root run and its siblings with it — so spans are validated against `VALID_RUN_TYPES` before being opened.
