...
async def test_create_with_a_custom_prompt_succeeds_and_echoes_it_back(
    client: AsyncClient,
) -> None:
    organization_id, workspace_id, assistant_id, owner_headers = await _setup_assistant(
        client,
        "asstver-customprompt",
    )

    response = await client.post(
        _versions_url(organization_id, workspace_id, assistant_id),
        json={**_VALID_PAYLOAD, "custom_prompt": "You are a friendly receptionist."},
        headers=owner_headers,
    )

    assert response.status_code == 201
    assert response.json()["custom_prompt"] == "You are a friendly receptionist."


async def test_create_rejects_an_overlong_custom_prompt(client: AsyncClient) -> None:
    organization_id, workspace_id, assistant_id, owner_headers = await _setup_assistant(
        client,
        "asstver-customprompt-toolong",
    )

    response = await client.post(
        _versions_url(organization_id, workspace_id, assistant_id),
        json={**_VALID_PAYLOAD, "custom_prompt": "x" * 8001},
        headers=owner_headers,
    )
