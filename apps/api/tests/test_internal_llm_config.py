...
async def test_returns_the_published_versions_config(
    client: AsyncClient, db: AsyncSession
) -> None:
    assistant = await _make_assistant(db, "internal-llm-config-ok")
    ...
    response = await client.get(
        _LLM_CONFIG_URL.format(assistant_id=assistant.id),
...
async def test_returns_the_default_config_for_an_unpublished_assistant(
    client: AsyncClient, db: AsyncSession
) -> None:
    assistant = await _make_assistant(db, "internal-llm-config-unpublished")
...
async def test_404s_for_an_unknown_assistant(client: AsyncClient) -> None:
    response = await client.get(
        _LLM_CONFIG_URL.format(assistant_id=uuid.uuid4()),
        headers={"X-Internal-Secret": settings.internal_api_secret},
    )
...
async def test_401s_with_a_missing_secret_header(
    ...
    assistant = await _make_assistant(db, "internal-llm-config-no-header")

    response = await client.get(_LLM_CONFIG_URL.format(assistant_id=assistant.id))
...
async def test_401s_with_a_wrong_secret_header(
    ...
    assistant = await _make_assistant(db, "internal-llm-config-wrong-header")
